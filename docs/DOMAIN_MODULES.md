...
- Multi-TF loop: `TFS=15m,1h,4h SYMBOLS=BTCUSDT,ETHUSDT scripts/run_sentinel_live_loop.sh`
- Per-TF paths: `/tmp/metaos_snapshots/<SYMBOL>_<TF>/` and `/tmp/metaos_domain_events/<SYMBOL>_<TF>/`
- Summary output: `/tmp/metaos_domain_events/_summary/summary_<TS>.json` includes `items` per symbol+timeframe.
